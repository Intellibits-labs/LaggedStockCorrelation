"""
Data Source:
- stocks.csv from inputs/ directory
- Original data from NSE (National Stock Exchange) listings

Purpose: Stock Market Correlation Analysis with Lagged Returns
Input:
- stocks.csv containing daily stock price data
- max_lag = 120 days for correlation calculations

Output:
1. stock_correlation_matrix.csv - Full correlation matrix (excluding self-correlations)
2. categorized_correlations.csv - Correlations categorized as:
   - Strong Positive (> 0.5)
   - Strong Negative (< -0.5)
   - No Correlation (-0.1 to 0.1)
3. correlation_heatmap.png - Visual heatmap of stock correlations
4. top_correlations.csv - Top 20 stock pairs by absolute correlation

Description:
- Calculates lagged correlations between all stock pairs
- Generates correlation matrix with masked self-correlations
- Identifies strongly correlated and uncorrelated pairs
- Creates visualizations using seaborn heatmaps
